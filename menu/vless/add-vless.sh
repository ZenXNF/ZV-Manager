...
add_vless() {
    clear
    _sep
    _grad " BUAT AKUN VLESS" 0 210 255 160 80 255
    _sep
    echo ""

    if ! xray_installed; then
        print_error "Xray-core belum diinstall!"
        press_any_key; return
    fi

    # Pilih server (hanya vless/both/all)
    ...
    for conf in "${server_dir}"/*.conf; do
        ...
        # Tampilkan server yang support vless
        [[ "$tg_type" == "ssh" || "$tg_type" == "vmess" ]] && continue
        count=$((count + 1))
        snames+=("$NAME")
        sips+=("$IP")
    ...
    if [[ $count -eq 0 ]]; then
        print_error "Belum ada server VLESS. Tambah server dengan tipe 'vless' atau 'all'."
        press_any_key; return
    fi
    ...
    if [[ -z "$username" ]]; then
        username=$(gen_vless_username)
    else
        username=$(echo "$username" | tr -dc 'a-zA-Z0-9-_')
    fi

    if [[ -f "${VLESS_DIR}/${username}.conf" ]]; then
    ...
    echo ""
    print_info "Membuat akun VLESS di server ${sname}..."
    ...
    mkdir -p "$VLESS_DIR"
    cat > "${VLESS_DIR}/${username}.conf" << CONFEOF
USERNAME="${username}"
UUID="${uuid}"
DOMAIN="${domain}"
EXPIRED_TS="${exp_ts}"
...
CONFEOF
    ...
    local result; result=$(remote_vless_agent "$sname" add "$username" "$uuid" "$exp_days" "0" "0")
    if echo "$result" | grep -q "^ADD-OK\|^ADD-ERR.*sudah ada"; then
        print_ok "Akun VLESS berhasil dibuat!"
    else
    ...
    clear
    _sep
    _grad " AKUN VLESS BERHASIL DIBUAT" 0 210 255 160 80 255
    _sep
    echo ""
