#!/bin/bash
# ============================================================
#   ZV-Manager - Lock/Unlock Akun VLESS
# ============================================================
...
manage_vless_lock() {
    ...
    clear
    _sep
    _grad " ${label} AKUN VLESS" 0 210 255 160 80 255
    _sep
    echo ""
    ...
    if is_local_target; then
        ...
        if [[ "$action" == "lock" ]]; then
            remote_vless_agent "local" disable "$username" &>/dev/null
            print_ok "Akun '${username}' berhasil di-lock"
        else
            remote_vless_agent "local" enable "$username" &>/dev/null
        ...
        local result; result=$(remote_vless_agent "$target" "$cmd" "$username")
        local ok_key="${cmd^^}-OK"
