...
def text_akun_info(tipe: str, username: str, password: str, domain: str,
    ...
    return (
        ...
        f"  Host   : <code>{domain}</code>\n"
        f"  Port   : <code>80</code> atau <code>443</code>\n"
        f"  User   : <code>{username}</code>\n"
        f"  Pass   : <code>{password}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"📦 <b>Payload</b>\n"
        f"  SSH 22  : <code>CONNECT {domain}:22 HTTP/1.1[crlf]Host: {domain}[crlf][crlf]</code>\n"
        f"  SSH 500 : <code>CONNECT {domain}:500 HTTP/1.1[crlf]Host: {domain}[crlf][crlf]</code>\n"
        f"  SSH 40K : <code>CONNECT {domain}:40000 HTTP/1.1[crlf]Host: {domain}[crlf][crlf]</code>\n"
        f"  DB 109  : <code>CONNECT {domain}:109 HTTP/1.1[crlf]Host: {domain}[crlf][crlf]</code>\n"
        f"  DB 143  : <code>CONNECT {domain}:143 HTTP/1.1[crlf]Host: {domain}[crlf][crlf]</code>"
        f"{harga_line}"
    )
